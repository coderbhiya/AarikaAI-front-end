      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className={`bg-white/5 rounded-lg shadow-md p-6 mb-8`}>
          <form onSubmit={applyFilters} className="space-y-4 md:space-y-0 md:grid md:grid-cols-4 md:gap-4">
            ...
            <div>
              ...
              <select id="location" name="location" value={filters.location} onChange={handleFilterChange} className={`w-full px-4 py-2 border border-gray-600 bg-white/5 text-white rounded-md focus:ring-2 focus:ring-blue-500`}>
                ...
                {availableFilters.locations.map((location, index) => (
                  <option key={index} value={location} className={`bg-gray-700 text-white}`}>
                    {location}
                  </option>
                ))}
              </select>
            ...
            <div>
              ...
              <select id="employmentType" name="employmentType" value={filters.employmentType} onChange={handleFilterChange} className={`w-full px-4 py-2 border border-gray-600 bg-white/5 text-white rounded-md focus:ring-2 focus:ring-blue-500`}>
                ...
                {availableFilters.employmentTypes.map((type, index) => (
                  <option key={index} value={type} className={`bg-gray-700 text-white`}>
                    {type}
                  </option>
                ))}
              </select>
            ...
            <div className="flex items-end space-x-2">
              <button type="submit" className={`px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md transition-colors duration-300`}>
                Apply Filters
              </button>
              <button type="button" onClick={resetFilters} className={`px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white font-medium rounded-md transition-colors duration-300`}>
                Reset
              </button>
            </div>
          </form>
                ...
                <div key={job.id} className={`bg-white/5 rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow duration-300`}>
                  <div className="p-6">
                    ...
                    <div className="flex flex-wrap gap-2 mt-3 mb-4">
                      ...
                      {job.employmentType && <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-900 text-green-200`}>{job.employmentType}</span>}
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-4">
                      <Link to={`/job/${job.id}`} className={`text-blue-400 hover:text-blue-300 font-medium text-sm`}>
                        View Details
                      </Link>

                      {job.link && (
                        <a href={job.link} target="_blank" rel="noopener noreferrer" className={`px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-md transition-colors duration-300`}>
                          Apply Now
                        </a>
                      )}
                    </div>
        ...
        {!loading && jobs.length > 0 && (
          <div className="flex justify-center mt-8">
            <button onClick={() => changePage(pagination.page - 1)} disabled={pagination.page === 1} className={`px-4 py-2 mr-2 bg-gray-700 hover:bg-gray-600 text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-300`}>
              Previous
            </button>
            <span className={`px-4 py-2 bg-gray-800 border-gray-600 text-white  xt-gray-800'} border rounded-md`}>
              Page {pagination.page} of {pagination.totalPages}
            </span>
            <button onClick={() => changePage(pagination.page + 1)} disabled={pagination.page === pagination.totalPages} className={`px-4 py-2 ml-2 bg-gray-700 hover:bg-gray-600 text-white rounded-md disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-300`}>
              Next
            </button>
          </div>
        )}
